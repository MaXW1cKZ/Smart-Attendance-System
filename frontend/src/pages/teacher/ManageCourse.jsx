...
function SettingsTab({ course, onSaved, onDeleted }) {
  ...
  return (
    /* ── centered, max width, proper spacing ── */
        ...
        {sessions.map((s) => (
          <div key={s.id}>
            {editId === s.id ? (
              ...
              /* ── Normal row — buttons always visible ── */
...
function StudentsTab({ courseId }) {
  ...
  const handleRemove = async () => {
    try {
      /* ── ใช้ student_id (รหัสนักศึกษา) ไม่ใช่ db id ── */
      await api.delete(
        `/courses/${courseId}/students/${removeConfirm.student_id}`,
      );
      setStudents((prev) =>
        prev.filter((s) => s.student_id !== removeConfirm.student_id),
      );
